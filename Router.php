...
class Router
{
  ...
  private function authRoutes($routes, IRequest $request)
  {
    ...
    $request->requestUri = (str_replace(BASE_URL,"",$request->requestUri))==="" ?"/":str_replace(BASE_URL,"",$request->requestUri);
    foreach($keys as $key){
      if(strpos(strtolower($key) , 'auth:') !==false){
        $authenticated = explode("/",$request->requestUri)[1];
        if(Sessions::session()->session_exist($authenticated) && strpos($request->requestUri,'/login') !== false){
          header('Location:'.str_replace("login",'',$request->requestUri));
        }
        foreach($routes[$key] as $k=>$newRoute){
          ...
          if(Sessions::session()->session_exist($authenticated) && isset($routes[$key][$request->requestUri])){
            return $routes[$key];
          }else{
